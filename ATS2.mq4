...
//if((ReCountBuy>Lok)&&((ReBuyLots<BuyLots) || (ReBuyLots>BuyLots))){Print("Произошли изменения у ордеров buy,пересчитаем ТР в 0");CalculateTotalBuyTP();}
//if((ReCountSell>Lok)&&((ReSellLots<SellLots) || (ReSellLots>SellLots))){Print("Произошли изменения у ордеров sell,пересчитаем ТР в 0");CalculateTotalSellTP();}
         ...
         if(Bid>(LastSellPrice+Level*k*Point))
           {
            ...
            Print("Открываем 11-й ордер Sell");
      SearchLastSellLot();Lot=NormalizeDouble(LastSellLot*CoefLot,2); if (Lot>(CritLot*(AccountBalance()/DinamicDepo))){Lot=CritLot*(AccountBalance()/DinamicDepo); NormalizeDouble(Lot,2);}
